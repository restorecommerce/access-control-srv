...
grpcConfig['service'] = grpcConfig['services'][cfg.get('serviceNames:cis')] || 'io.restorecommerce.commandinterface.Service';
...
Object.keys(grpcConfig['services']).forEach((service) => {
  requests.push(new Promise<any>(async (resolve: any, reject) => {
    try {
